...
$(function () {
    ...
    //CEP PARTNER
    $("#zipcode_partner").blur(function () {
        var cep = $("#zipcode_partner").val().replace(/[^0-9]/, "");
        var url = "https://viacep.com.br/ws/" + cep + "/json/";
        $.getJSON(url, function (dadosRetorno) {

            $("#street_partner").val(dadosRetorno.logradouro);
            $("#district_partner").val(dadosRetorno.bairro);
            $("#state_partner").val(dadosRetorno.uf);
            $("#city_partner").val(dadosRetorno.localidade);
